...
<div class="card-dashboard mb-8">
    <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <a href="{{ route('dokter.booking') }}" class="text-xs font-semibold text-primary-600 hover:text-primary-700">Lihat Semua</a>
    </div>
    <div class="divide-y divide-gray-50">
        ...
        <div class="px-5 py-3 flex items-center justify-between hover:bg-gray-50 transition">
            <div class="flex items-center gap-3 min-w-0 flex-1">
                ...
                <div class="min-w-0">
                    <p class="text-sm font-medium text-gray-800 truncate">{{ $b->pasien->name ?? '-' }}</p>
                    <p class="text-xs text-gray-400">{{ $b->keluhan_awal ? \Illuminate\Support\Str::limit($b->keluhan_awal, 40) : 'Tanpa keluhan' }}</p>
                </div>
            </div>
            <div class="flex items-center gap-3 flex-shrink-0">
                <span class="badge-status badge-{{ $b->status }} text-xs">{{ ucfirst($b->status) }}</span>
                <span class="text-xs text-gray-400 font-medium">{{ \Carbon\Carbon::parse($b->jam_booking)->format('H:i') }}</span>
            </div>
        </div>
        @endforeach
    </div>
...
<div class="card-dashboard">
    <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <a href="{{ route('dokter.antrian') }}" class="text-xs font-semibold text-primary-600 hover:text-primary-700">Kelola Antrian</a>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full text-left">
            <thead>
                <tr class="bg-gray-50">
                    <th class="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">No. Antrian</th>
                    <th class="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Pasien</th>
                    <th class="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Keluhan</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="px-5 py-3">
                        <span class="text-sm font-bold text-gray-900">{{ $a->nomor_antrian }}</span>
                    </td>
                    <td class="px-5 py-3">
                        <div class="flex items-center gap-2.5">
                            <div class="w-8 h-8 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center text-xs font-bold flex-shrink-0 overflow-hidden">
                                @if($a->pasien->foto)
                            ...
                            <div>
                                <p class="text-sm font-medium text-gray-800">{{ $a->pasien->user->name ?? '-' }}</p>
                                <p class="text-xs text-gray-400">{{ $a->pasien->no_telp ?? '-' }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="px-5 py-3">
                        <p class="text-sm text-gray-600 max-w-[200px] truncate">{{ $a->complaint ?? '-' }}</p>
                    </td>
                    <td class="px-5 py-3">
                    ...
                    <td class="px-5 py-3">
                        ...
                        @else
                        <span class="text-xs text-gray-400">{{ $a->status == 'selesai' ? 'Selesai' : ($a->status == 'dibatalkan' ? 'Batal' : '-') }}</span>
                        @endif
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="5" class="px-5 py-10 text-center">
                        <p class="text-gray-400 text-sm">Belum ada antrian hari ini.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
