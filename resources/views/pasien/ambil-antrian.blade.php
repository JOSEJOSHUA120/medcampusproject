...
<div class="page-header">
    <h4>Ambil Antrian</h4>
    <p>Pilih dokter untuk mengambil nomor antrian. Dokter yang tidak tersedia akan tampak redup dan tidak bisa dipilih.</p>
</div>
    ...
    <form action="{{ route('pasien.antrian.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="dokter_id" class="form-select-custom @error('dokter_id') border-red-400 @enderror" required>
                ...
                @foreach($dokter as $d)
                @php $avail = $d->isAvailable(); @endphp
                <option value="{{ $d->id }}" {{ !$avail ? 'disabled' : '' }}>
                    dr. {{ $d->user->name ?? $d->nama_dokter }} ({{ $d->spesialisasi }}) {{ !$avail ? '-- Tidak Tersedia' : '' }}
                </option>
                @endforeach
            </select>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    @foreach($dokter as $d)
    @php $avail = $d->isAvailable(); @endphp
    {{-- Card dokter: jika tidak tersedia, gunakan opacity rendah --}}
    <div class="card-dashboard p-5 {{ !$avail ? 'opacity-50' : '' }}">
        ...
        <p class="text-xs text-gray-400">{{ $d->getJadwalText() }}</p>
        @if(!$avail)
        <div class="mt-2 inline-block px-2 py-0.5 bg-red-100 text-red-600 text-xs rounded-full font-medium">
            Tidak Tersedia
        </div>
        @else
